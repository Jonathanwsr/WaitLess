...

{
    ...
    public function mostrarAvaliacoes(Request $request, $id)
    {
        ...
        try {
            ...
            $avaliacoesQuery = Avaliacao::with([
                    ...
                    'estabelecimento:id,name as nome' // Garantindo alias para estabelecimentos
                ])
                ->where('publica', true)
            ...
            $avaliacoes = $avaliacoesQuery->paginate(15)->withQueryString();
            $totalAvaliacoes = $avaliacoes->total();

            $estatisticas = [
                'total' => $totalAvaliacoes,
                'media_geral' => $totalAvaliacoes > 0 ? (clone $avaliacoesQuery)->avg('nota') : 0,
                'estrelas' => [
                ...
                'categorias' => [
                    'limpeza'         => (clone $avaliacoesQuery)->avg('nota_limpeza') ?? 0,
                    'precisao'        => (clone $avaliacoesQuery)->avg('nota_precisao') ?? 0,
                    'comunicacao'     => (clone $avaliacoesQuery)->avg('nota_comunicacao') ?? 0,
                    'localizacao'     => (clone $avaliacoesQuery)->avg('nota_localizacao') ?? 0,
                    'checkin'         => (clone $avaliacoesQuery)->avg('nota_checkin') ?? 0,
                    'custo_beneficio' => (clone $avaliacoesQuery)->avg('nota_custo_beneficio') ?? 0,
                ]
            ];
            ...
            return Inertia::render('Cliente/Avaliacoes', [
                ...
                'mensagem' => 'O item, serviço ou estabelecimento que você tentou acessar não foi encontrado.'
            ]);
        }
    }

    // =========================================================================
    // PERMITIR DENÚNCIA A PARTIR DA TELA EXPLORAR 
    // =========================================================================
    public function denunciar(Request $request, $id)
    {
        ...
        return back()->with('success', 'Avaliação denunciada com sucesso. Nossa equipe de moderação irá analisar.');
    }
