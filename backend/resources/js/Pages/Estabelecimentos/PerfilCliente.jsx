...
import { 
  ...
  FileText, CreditCard, User, Clock, CheckCircle2, XCircle 
} from 'lucide-react';

export default function PerfilCliente({ auth, cliente = {}, agendamentos = [], pagamentos = [] }) {
  ...
  return (
    <AuthenticatedLayout user={auth.user}>
      <Head title={`Perfil - ${cliente.name || 'Cliente'}`} />
      ...
      <div className="min-h-screen bg-slate-50/60 pb-12 font-sans antialiased text-slate-800">
        ...
        >
          {/* Camada escura de sobreposição para leitura do botão */}
          <div className="absolute inset-0 bg-black/40"></div>

          {/* Botão Voltar posicionado à esquerda e acima da camada escura (z-10) */}
          <div className="absolute top-6 left-4 sm:left-8 lg:left-12 z-10">
            <button
              onClick={() => window.history.back()}
              className="inline-flex items-center gap-2 px-3.5 py-2 bg-white/20 hover:bg-white/30 text-white backdrop-blur-md rounded-xl transition-all duration-200 text-sm font-medium border border-white/20 shadow-sm"
            >
              <ArrowLeft size={18} />
              <span>Voltar</span>
            </button>
