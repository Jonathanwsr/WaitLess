...
export default function ClienteDashboard({ auth, agendamentos = [], historico = [], usuario }) {
    ...
    historico.forEach(ag => {
        if (ag.status === 'concluido' || ag.status === 'finalizado') {
            agsConcluidos.push(ag);
        } else if (ag.status === 'cancelado' || ag.status_pagamento === 'estornado') {
            agsCancelados.push(ag);
        } else {
            agsConcluidos.push(ag); // Fallback caso venha algo estranho do banco
        }
    });
                    ...
                    {agendamentosExibidos.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                            {agendamentosExibidos.map((agendamento) => {
                                ...
                                const isCanceladoDefinitivo = agendamento.status === 'cancelado' || statusTempo.cancelado || isEstornado || (statusTempo.expirou && !isConfirmado && !isConcluido);
                                const isAguardandoPagamento = agendamento.status === 'aguardando_pagamento' && !isCanceladoDefinitivo && !isConfirmado && !isConcluido;
                                
                                const isEmAndamento = agendamento.status === 'em_atendimento' || agendamento.status === 'em_andamento' || agendamento.em_andamento;
                                const foiChamado = agendamento.status === 'confirmado'; 

                                return (
                                    <div key={agendamento.id} className={`rounded-[2rem] bg-white border p-6 flex flex-col justify-between transition-all duration-300 ${isEmAndamento || foiChamado ? 'border-[#E05D36] shadow-[0_8px_30px_rgba(224,93,54,0.15)] ring-1 ring-[#E05D36]' : 'border-gray-100 shadow-[0_4px_25px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_35px_rgba(0,0,0,0.08)] hover:-translate-y-1'}`}>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex justify-between items-start mb-6 gap-2">
                                                <div className="flex-shrink-0 mt-1">
                                                    {foiChamado ? (
                                                        <span className="bg-[#E05D36] text-white text-[10px] uppercase tracking-wider font-black px-3 py-1.5 rounded-lg shadow-md animate-pulse">Sua Vez Chegou!</span>
                                            ...
                                            {isConfirmado && agendamento?.codigo_verificacao && !isConcluido && !isEmAndamento && abaAtiva === 'proximos' && (
                                                <div className={`mb-6 text-white rounded-[1.25rem] p-4 text-center shadow-md relative overflow-hidden ${foiChamado ? 'bg-[#E05D36] animate-pulse border-2 border-[#C74B27]' : 'bg-gray-900'}`}>
                                                    <div className="absolute top-0 right-0 w-16 h-16 bg-white/10 rounded-bl-full"></div>
                                                    <p className="text-[10px] font-black text-gray-200 uppercase tracking-widest mb-1 relative z-10">{foiChamado ? 'APRESENTE ESSE PIN AGORA!' : 'Código de Check-in'}</p>
                                        ...
                                        <div className="mt-4 space-y-3">
                                            {(() => {
                                                ...
                                                if (isConcluido) {
                                                    return (
                                                        <>
                                                            {agendamento.pode_avaliar && (
                                                                <Link 
                                                                    href={`/avaliacoes/${agendamento.estabelecimento_id}?tipo=estabelecimento&agendamento=${agendamento.id}`} 
                                                                    className="w-full flex justify-center items-center gap-2 py-4 bg-white border-2 border-[#E05D36] text-[#E05D36] rounded-2xl text-sm font-bold hover:bg-[#FFF2EE] transition-all shadow-sm"
                                                                >
                                                                    <StarIcon className="w-5 h-5" /> Avaliar Experiência
                                                                </Link>
                                                            )}
                                                            <a 
                                                                href={`/agendamentos/${agendamento.id}/comprovante-pdf`} 
                                                                target="_blank"
                                                                rel="noopener noreferrer"
                                                                className="w-full flex justify-center items-center gap-2 py-3 bg-gray-50 border border-gray-200 text-gray-900 rounded-2xl text-sm font-bold hover:bg-gray-100 transition-all"
                                                            >
                                                                <DocumentArrowDownIcon className="w-4 h-4" /> Baixar Comprovante
                                                            </a>
                                                            <Link href={`/estabelecimentos/${agendamento.estabelecimento_id}`} className="w-full flex justify-center items-center gap-2 py-3 bg-indigo-50 text-indigo-600 rounded-2xl text-sm font-bold hover:bg-indigo-100 transition-all">
                                                                <ArrowPathIcon className="w-4 h-4" /> Reagendar Serviço
                                                            </Link>
                                                        </>
                                                    );
                                                }
                                                // 4. SE CANCELADO DEFINITIVO
                                                if (isCanceladoDefinitivo) {
                                                    return (
                                                ...
                                                return (
                                                    <button 
                                                        ...
                                                        className="w-full flex justify-center items-center gap-2 py-4 bg-gray-50 border border-gray-200 text-gray-900 rounded-2xl text-sm font-bold hover:border-gray-300 hover:bg-gray-100 transition-all"
                                                    >
                                                        Ver Detalhes
                                                    </button>
                                                );
